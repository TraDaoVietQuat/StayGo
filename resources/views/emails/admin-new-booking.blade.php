...
<head>
<meta charset="UTF-8">
<title>Đặt phòng mới - StayGo Admin</title>
<style>
...
.hdr{background:#1B3A6B;padding:20px 28px;display:flex;align-items:center;gap:12px;}
...
<div class="wrap">
  ...
  <div class="body">
    <div class="row"><span class="lbl">Mã đơn</span><span class="val" style="color:#1B3A6B;letter-spacing:1px;">{{ $booking->order_code }}</span></div>
    ...
    <div class="row"><span class="lbl">Thanh toán</span><span class="val">{{ $booking->payment_method }}</span></div>
    <div class="row"><span class="lbl">Tổng tiền</span><span class="val total">{{ number_format($booking->total_price, 0, ',', '.') }}đ</span></div>
    <div class="row"><span class="lbl">Thời điểm đặt</span><span class="val">{{ now()->format('H:i d/m/Y') }}</span></div>
  </div>
  <div class="footer">
